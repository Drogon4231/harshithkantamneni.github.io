...
// -----------------------------
// Core data – exact titles + hiring-manager-focused bullets
// -----------------------------
const projects = [
  {
    title: "ML-Guided CUDA Kernel Configuration",
    bullets: [
      "Engineered a PyTorch mini-MLP that predicts grid/block sizes on-the-fly, boosting average GEMM throughput 30% on NVIDIA A100 GPUs.",
      "Replaced exhaustive grid-search with one inference call—cutting kernel tuning time >95% and saving ~6 GPU-hours per dataset.",
      "Deployed across Euler cluster via Slurm scripts; codebase forked by peers for benchmark suites.",
    ],
  ...
  },
  {
    ...
    bullets: [
      "Trained XGBoost regressor on 2 000 task graphs to predict runtime-optimal partition sizes with <5 % MAE.",
      "Accelerated simulation pipeline by 25 % versus exhaustive sweeps, enabling nightly CI performance checks.",
      "Packaged as Python API + CLI; adopted as reference tooling for future ECE 757 cohorts.",
    ],
    link: "https://github.com/Drogon4231/ML-Partition-Predictor",
  },
  {
    title: "5-Stage Pipelined RISC Processor (WISC-F24 ISA)",
    bullets: [
      "Designed hazard-free pipeline in Verilog with full forwarding and branch prediction.",
  ...
  },
  {
    ...
    bullets: [
      "Implemented pipelined state machine; synthesizes to 333 MHz on Artix-7.",
  ...
  },
  {
    ...
    bullets: [
      "Built FreeRTOS app on PSoC 6 interfacing SCD41 + MQ-7 via I²C and ADC.",
      "Designed 4-layer sensor-fusion PCB in Altium; streams real-time data over Ethernet.",
    ],
    link: "#",
  },
];
      ...
      <main className="pt-24 space-y-32 overflow-x-hidden">
        ...
        <section id="skills" className="max-w-4xl mx-auto px-6">
          ...
          <ul className="flex flex-wrap gap-3">
            {skills.map((s, idx) => (
              <li key={idx} className="px-3 py-1 rounded-full bg-emerald-500/10 text-sm text-emerald-600 dark:text-emerald-400">{s}</li>
            ))}
          </ul>
        </section>
        ...
        <section id="contact" className="max-w-md mx-auto text-center px-6">
          ...
          <p className="mb-6 text-lg">Shoot me an email if you have an opportunity, idea, or want to geek out about GPUs.</p>
          <a href="mailto:hkantamneni2@wisc.edu" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white font-medium shadow-lg hover:shadow-xl transition-shadow"><Mail size={18} /> Say Hello</a>
          <div className="flex justify-center gap-6 mt-8">
            <a href="https://github.com/harshith" target="_blank" rel="noopener noreferrer" className="hover:text-emerald-500"><Github size={22} /></a>
            <a href="https
